...
- (NSString *)yq_timeInfoWithShowDateInToday:(BOOL)showDateInToday {
    return [NSDate yq_timeInfoWithDate:self showDateInToday:showDateInToday];
}

+ (NSString *)yq_timeInfoWithDate:(NSDate *)date showDateInToday:(BOOL)showDateInToday {
    return [self yq_timeInfoWithDateString:[self yq_stringWithDate:date format:[self yq_ymdHmsFormat]] showDateInToday:showDateInToday];
}

+ (NSString *)yq_timeInfoWithDateString:(NSString *)dateString showDateInToday:(BOOL)showDateInToday {
    ...
    if (month == 0 && year == 0 && day < 2) {
        if (showDateInToday) {
            if (day == 0) {
                return [date yq_stringWithFormat:[self yq_hmsFormat]];
            } else if (day == 1) {
                return @"昨天";
            }
