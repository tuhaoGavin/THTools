...
#import "NSObject+Runtime_th.h"
#import <objc/runtime.h>
BOOL th_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void th_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void th_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (Runtime_th)

+ (void)th_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    th_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)th_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    th_method_append(self.class, klass, newMethod);
}

+ (void)th_replaceMethod:(SEL)method fromClass:(Class)klass
{
    th_method_replace(self.class, klass, method);
}

- (BOOL)th_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class th_instancesRespondToSelector:selector untilClass:stopClass];
}
...
{
    return [self.superclass th_instancesRespondToSelector:selector untilClass:stopClass];
}
