...
@implementation NSString (Emoji_th)

static NSDictionary * th_s_unicodeToCheatCodes = nil;
static NSDictionary * th_s_cheatCodesToUnicode = nil;

+ (void)th_initializeEmojiCheatCodes
{
    ...
    @synchronized(self) {
        th_s_unicodeToCheatCodes = forwardMap;
        th_s_cheatCodesToUnicode = [reversedMap copy];
    }
}

- (NSString *)th_stringByReplacingEmojiCheatCodesWithUnicode
{
    if (!th_s_cheatCodesToUnicode) {
        [NSString th_initializeEmojiCheatCodes];
    }
    
    if ([self rangeOfString:@":"].location != NSNotFound) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [th_s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
...
- (NSString *)th_stringByReplacingEmojiUnicodeWithCheatCodes
{
    ...
    if (self.length) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [th_s_unicodeToCheatCodes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
