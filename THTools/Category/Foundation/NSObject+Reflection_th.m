...
@implementation NSObject (Reflection_th)
...
{
    return [[self class] th_propertyKeys];
}
+ (NSArray *)th_propertyKeys {
...
{
    return [[self class] th_propertiesInfo];
}
/**
...
+ (NSArray *)th_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self th_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)th_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] th_propertiesInfo];
...
-(NSArray*)th_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] th_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] th_decodeType:returnType];
        NSString *encodeString = [[self class] th_decodeType:encoding];
...
-(NSDictionary *)th_protocolList{
    return [[self class]th_protocolList];
}
+ (NSDictionary *)th_protocolList
{
...
+ (NSArray *)th_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] th_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)th_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)th_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString th_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
