...
@implementation UIButton (CountDown_yq)
    ...
    dispatch_source_set_event_handler(self.countDownTimer, ^{
        if (timeOut<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
//                NSLog(@"____%@",strTime);
                [self setTitle:[NSString stringWithFormat:@"%@%@%@", preTittle, [NSString yq_getMMSSFromSS:strTime], sufTitle] forState:UIControlStateNormal];
                self.enabled = disEnableWhenCountDown ? NO : YES;
                self.isCountDown = YES;
            });
            timeOut--;
