...
@implementation UIDevice (Hardware_th)
...
+ (BOOL)isIPhoneXSeries {
    ...
    if (@available(iOS 11.0, *)) {
        ...
        if (UIApplication.sharedApplication.delegate.window.safeAreaInsets.bottom > 0.0f) {
            flag = YES;
        }
    }
    return flag;
...
+ (NSString *)th_platformString{
    NSString *platform = [self th_platform];
    ...
    if ([platform isEqualToString:@"iPad7,1"])      return @"iPad Pro 12.9-inch 2 (WiFi)";
    if ([platform isEqualToString:@"iPad7,2"])      return @"iPad Pro 12.9-inch 2 (Cellular)";
...
+ (NSUInteger)th_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)th_cpuFrequency {
    return [self th_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)th_busFrequency {
    return [self th_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)th_ramSize {
    return [self th_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)th_cpuNumber {
    return [self th_getSysInfo:HW_NCPU];
}


+ (NSUInteger)th_totalMemoryBytes
{
    return [self th_getSysInfo:HW_PHYSMEM];
}
